...
Last edited on April 16, 2021 by Swapna Sindhu Mishra
"""
...
downswitch=-40 #downswitch field value
upswitch=40 #upswitch field value
...
downrangeHH=downH[downH > downswitch - 250]
uprangeHH=upH[upH < upswitch + 250]

#plt.title(title, fontsize=20)
plt.ylabel('$I_c$ (mA)', fontsize=20)
plt.yticks(fontsize=15)
plt.xlabel('$\mu_0 H$ (mT)', fontsize=20)
plt.xticks(fontsize=15)
plt.xlim(-90, 90)
plt.ylim(-0.001, 0.05)
plt.scatter(downdata[:,0]/10,downdata[:,2], s=10, color='b' )
plt.scatter(updata[:,0]/10,updata[:,2], s=10, color='r', facecolors='none',  alpha=.6, marker="s")
plt.plot(downrangeHH/10,fit_func_4(downrangeHH,-downparams[0],downparams[1],downparams[2]),color='b', linewidth=1)
#, label=f'$I_C R_N$ = {downfit_IcRn} $\mu V$'
plt.plot(uprangeHH/10,fit_func_4(uprangeHH,upparams[0],upparams[1],upparams[2]), alpha=1, color='r', linewidth=1)
#, label=f'$I_C R_N$ = {upfit_IcRn} $\mu V$'
#plt.legend(loc='best', fontsize=10.5)
plt.savefig(f'{sample}_Airy_Fit_Extended.svg', bbox_inches='tight')
plt.savefig(f'{sample}_Airy_Fit_Extended.png', bbox_inches='tight', dpi=600)
plt.show()
